...
const videoData = (user, videoId) => {
  ...
  service
    .get(
      `/weixin/shortVideo/videoData?${user.paramsString}&type=1&videoId=${videoId}&videoShareId=`
    )
    .then(res => {
      viewVideoCount += 1;
      const data = res.data;
...
const setVideoOpera = async (user, playId, needLike = true) => {
  ...
  if (needLike) {
    await service.get(
      `/weixin/shortVideo/setVideoOpera?toPage=0&${user.paramsString}&state=N&type=1&playId=${playId}`
    );
    service
      .get(
        `/weixin/shortVideo/setVideoOpera?toPage=0&${user.paramsString}&state=Y&type=1&playId=${playId}`
      )
      .then(res => {
        const data = res.data;
...
const getVideoList = (user, num) => {
  ...
  service
    .get(
      `/weixin/shortVideo/getVideoList?toPage=8&${paramsString}&queryType=2&num=${num}&sortType=3`
    )
    .then(res => {
      const data = res.data;
      if (data.errorCode === "901") {
        ...
        console.log(
          JSON.stringify(videoList.map(v => ({ id: v.id, name: v.name })))
        );
      }
    });
};
...
const newLogin = user => {
  service
    .get(
      `/ssoauth/NewLogin?code=a547e63aec2c8ca4e6daa97a6f52c9a9&sc=11&spid=949a91f46a4d1da7016a90ea14e00002`
    )
    .then(res => {
      console.log(res);
...
const miniAppManager = user => {
  service
    .get(
      `/mobileStore/miniappsNew/miniAppManager.do?${user.paramsString}&version=2&businessIds=wx_bt00001%2Cwx_bt00002`
    )
    .then(res => {
      console.log(res);
      console.log(JSON.stringify(res.headers));
...
const Users = {
  lzy: {
    paramsString:
      "nonce=km9860bb7if2eq4b&encpn=da5cae5849566abde0b6c6a85de0812a&cf=10113&session=DKN0VKMUEVOB22N1FEXM197G"
  },
  xlz: {
    paramsString:
      "nonce=57Z8Onb7IrVR38Vk&encpn=29371a899ef6e659eebc1afdbe8ea774&cf=10113&session=NM4QDV9EVP98BDZD4UCWRDYJ"
  }
};
